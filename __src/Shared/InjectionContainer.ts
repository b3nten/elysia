function mapPredicate<K, V>(
	map: Map<K, V>,
	predicate: (key: K, value: V) => boolean,
): [K, V][] {
	return Array.from(map)
		.filter(([key, value]) =>
			predicate(key, value) ? [key, value] : undefined,
		)
...
export class InjectionContainer {
	...
	registerTransient<
		K extends Key<any> | Constructor<any>,
	...
	registerSingleton<K extends Key<any>, T extends Constructor<any>>(
		key: K | T,
		value?: T,
	) {
	...
	#resolveTransient<T>(key: Key<T>): T | undefined {
		const instance =
			...
			mapPredicate(
				this.instances,
				(k) => typeof k === "function" && key instanceof k,
			)[0]?.[1];
		if (instance) {
			try {
				return new instance();
	...
	resolve<T>(key: Key<T>): T {
		...
		try {
			const value =
				this.#resolveSingleton(key) ??
				this.#resolveTransient(key) ??
