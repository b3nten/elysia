...
class BatchedLodMesh extends Mesh {
	...
	_validateGeometry(geometry) {
		...
		if (Boolean(geometry.getIndex()) !== Boolean(batchGeometry.getIndex())) {
			throw new Error(
				'THREE.BatchedLodMesh: All geometries must consistently have "index".',
			);
		}
		...
		for (const attributeName in batchGeometry.attributes) {
			...
			if (
				...
				throw new Error(
					"THREE.BatchedLodMesh: All attributes must have a consistent itemSize and normalized value.",
				);
			}
		}
	}
	...
	addGeometry(geometry, reservedVertexCount = -1, reservedIndexCount = -1) {
		...
		if (
			...
			throw new Error(
				"THREE.BatchedLodMesh: Reserved space request exceeds the maximum buffer size.",
			);
		}
	...
	setGeometryAt(geometryId, geometry) {
		...
		if (
			...
			throw new Error(
				"THREE.BatchedLodMesh: Reserved space not large enough for provided geometry.",
			);
		}
