...
class ShootBallBehavior extends Elysia.Core.Behavior {
	...
	override onCreate() {
		this.app.input.onKeyDown(MouseCode.MouseLeft, () => {
			...
			const ball = new Elysia.Actors.Mesh(
				ShootBallBehavior.Geometry,
				ShootBallBehavior.Material,
			);
			...
			(
				this.scene.physics as unknown as PhysicsWorldComponent
			).world!.bodyInterface.AddImpulse(
				body.joltBodyID!,
				new Jolt.Vec3(
					this.scene.activeCamera.getWorldDirection(new Three.Vector3()).x *
						400,
					this.scene.activeCamera.getWorldDirection(new Three.Vector3()).y *
						400,
					this.scene.activeCamera.getWorldDirection(new Three.Vector3()).z *
						400,
				),
			);
		});
