...
const RedIntegerFormat = Three.RedIntegerFormat;
const UnsignedIntType = Three.UnsignedIntType;
...
/** @type { import("three").BatchedMesh } */
class BatchedLodMesh extends Mesh {
	...
	_validateGeometry( geometry ) {
		...
		if ( Boolean( geometry.getIndex() ) !== Boolean( batchGeometry.getIndex() ) ) {
			...
			throw new Error( 'THREE.BatchedLodMesh: All geometries must consistently have "index".' );
		...
		for ( const attributeName in batchGeometry.attributes ) {
			...
			if ( srcAttribute.itemSize !== dstAttribute.itemSize || srcAttribute.normalized !== dstAttribute.normalized ) {
				...
				throw new Error( 'THREE.BatchedLodMesh: All attributes must have a consistent itemSize and normalized value.' );
	...
	addInstance( geometryId ) {
		...
		if ( atCapacity && this._availableInstanceIds.length === 0 ) {
			...
			throw new Error( 'THREE.BatchedLodMesh: Maximum item count reached.' );
	...
	addGeometry( geometry, reservedVertexCount = - 1, reservedIndexCount = - 1 ) {
		...
		if (
			...
			throw new Error( 'THREE.BatchedLodMesh: Reserved space request exceeds the maximum buffer size.' );
	...
	setGeometryAt( geometryId, geometry ) {
		...
		if ( geometryId >= this._geometryCount ) {
			...
			throw new Error( 'THREE.BatchedLodMesh: Maximum geometry count reached.' );
		...
		if (
			...
			throw new Error( 'THREE.BatchedLodMesh: Reserved space not large enough for provided geometry.' );
