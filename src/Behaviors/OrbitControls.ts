...
class ActorAdaptor {
	get matrix() {
		return this.actor.localMatrix;
	}
	get matrixWorld() {
		return this.actor.worldMatrix;
	}
	get position() {
	...
	get rotation() {
		return this.euler.setFromQuaternion(this.quaternion);
	}
	get scale() {
		return this.actor.scale;
	}
	get up() {
		return this.actor.object3d.up;
	}
	...
	lookAt(x: number | Three.Vector3, y: number, z: number) {
		...
		if (
			this.actor.object3d instanceof Three.Camera ||
			this.actor.object3d instanceof Three.Light
		) {
...
class CameraActorAdaptor extends ActorAdaptor {
	get isOrthographicCamera() {
		return this.actor.object3d instanceof Three.OrthographicCamera;
	}
	get isPerspectiveCamera() {
		return this.actor.object3d instanceof Three.PerspectiveCamera;
	}
	get projectionMatrix() {
		return (this.actor.object3d as Three.PerspectiveCamera).projectionMatrix;
	}
	get projectionMatrixInverse() {
		return (this.actor.object3d as Three.PerspectiveCamera)
			.projectionMatrixInverse;
	}
	get fov() {
		return (this.actor.object3d as Three.PerspectiveCamera).fov;
	}
	set fov(value: number) {
		(this.actor.object3d as Three.PerspectiveCamera).fov = value;
	}
	get aspect() {
		return (this.actor.object3d as Three.PerspectiveCamera).aspect;
	}
	set aspect(value: number) {
		(this.actor.object3d as Three.PerspectiveCamera).aspect = value;
	}
	get near() {
		return (this.actor.object3d as Three.PerspectiveCamera).near;
	}
	set near(value: number) {
		(this.actor.object3d as Three.PerspectiveCamera).near = value;
	}
	get far() {
		return (this.actor.object3d as Three.PerspectiveCamera).far;
	}
	set far(value: number) {
		(this.actor.object3d as Three.PerspectiveCamera).far = value;
	}
...
export class OrbitControls extends Behavior {
	...
	setControlSetting(
		key: NonFunctionPropertyNames<OrbitControlsImpl>,
		value: any,
	) {
	...
	override onCreate() {
		...
		if (!isThreeActor(parent))
			throw Error("OrbitControls must be attached to a ThreeObject");

		const camera = this.parent as ThreeObject<Three.Camera>;

		if (!camera) {
			ELYSIA_LOGGER.error(
				"No camera found to attach orbit controls to",
				this.parent,
			);
			return;
		}
		...
		this.controls = new OrbitControlsImpl(
			adaptor,
			this.#target ?? this.app.renderPipeline.getRenderer().domElement,
		);
