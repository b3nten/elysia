...
export class Player extends Actor
{
	...
	override onCreate()
	{
		...
		if(!Jolt) throw new Error("Jolt physics engine not initialized when trying to create a Player.");
		...
		this.shape = new Jolt.RotatedTranslatedShapeSettings(positionStanding, rotation, new Jolt.CapsuleShapeSettings(0.5 * characterHeightStanding, characterRadiusStanding)).Create().Get();
	...
	override onBeforePhysicsUpdate(delta: number, elapsed: number): void
	{
		...
		let forward = input.isDown(KeyCode.W) ? 1.0 : (input.isDown(KeyCode.S) ? -1.0 : 0.0);
		let right = input.isDown(KeyCode.D) ? 1.0 : (input.isDown(KeyCode.A) ? -1.0 : 0.0);
		let jump = input.isDown(KeyCode.Space);

		const cameraWorldQuat = new Three.Quaternion;
		this.camera.worldMatrix.decompose(new Three.Vector3, cameraWorldQuat, new Three.Vector3);

		const movementDirection = new Three.Vector3(right, 0, -forward).applyQuaternion(cameraWorldQuat);
		...
		const playerControlsHorizontalVelocity = this.controlMovementInAir || this.character!.character!.IsSupported();
		if (playerControlsHorizontalVelocity) {
			// True if the player intended to move
			allowSliding = !(movementDirection.length() < 1.0e-12);
			// Smooth the player input
			if (enableCharacterInertia) {
				this.desiredVelocity.multiplyScalar(0.75).add(movementDirection.multiplyScalar(0.25 * this.characterSpeed))
			} else {
