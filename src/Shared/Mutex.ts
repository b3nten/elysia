...
export class Mutex {
	...
	constructor(backingArrayBuffer?: SharedArrayBuffer | ArrayBuffer) {
		this.backingArrayBuffer =
			backingArrayBuffer || Mutex.SupportsSharedArrayBuffer
	...
	lock() {
		for (;;) {
			if (
				Atomics.compareExchange(this.#value, 0, MUTEX_UNLOCKED, MUTEX_LOCKED) ==
				MUTEX_UNLOCKED
			) {
				return;
			}
	...
	unlock() {
		if (
			Atomics.compareExchange(this.#value, 0, MUTEX_LOCKED, MUTEX_UNLOCKED) !=
			MUTEX_LOCKED
		) {
			throw new Error(
